...
void AddAdditionalLights_float(UnityTexture2D posterization, float Smoothness, float3 WorldPosition, float3 WorldNormal, float3 WorldView,
    ...
    for (int i = 0; i < pixelLightCount; ++i) {
        ...
        float3 posterizedCol = tex2D(posterization, float2(thisDiffuse * 70, 0.5f));
        if (thisDiffuse > 0)
        {
